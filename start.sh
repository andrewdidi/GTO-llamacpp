...
# 可写数据默认全部落在 VOLUME_ROOT（/models 挂载盘）
set -euo pipefail
...
_SAVED_VOLUME_ROOT="${VOLUME_ROOT-}"
_SAVED_CACHE_ROOT="${CACHE_ROOT-}"
...
[[ -n "${_SAVED_VOLUME_ROOT}" ]] && VOLUME_ROOT="$_SAVED_VOLUME_ROOT"
[[ -n "${_SAVED_CACHE_ROOT}" ]] && CACHE_ROOT="$_SAVED_CACHE_ROOT"
...
source "${ROOT}/scripts/volume_layout.sh"
prepare_volume_layout || exit 1

export HF_TOKEN="${HF_TOKEN:-${HUGGING_FACE_HUB_TOKEN:-}}"
export HUGGING_FACE_HUB_TOKEN="${HUGGING_FACE_HUB_TOKEN:-$HF_TOKEN}"
export HF_REPO_ID="${HF_REPO_ID:-}"
export HF_LOCAL_DIR MODEL_PATH MODEL_URL VOLUME_ROOT CACHE_ROOT LOG_DIR
export HF_GGUF_REPO="${HF_GGUF_REPO:-unsloth/Qwen3.5-9B-GGUF}"
export HF_GGUF_FILE="${HF_GGUF_FILE:-Qwen3.5-9B-Q4_K_M.gguf}"
export AUTO_CONVERT="${AUTO_CONVERT:-0}"
